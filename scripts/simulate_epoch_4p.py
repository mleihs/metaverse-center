...
def game_01(token):
    ...
    last = run_foundation(epoch_id, players, admin, {"V": 6, "CK": 0, "SN": 2, "SP": 2})

    def strat(eid, pl, cyc):
        # CK (0 guardians) attacks everyone
        for t_tag in ["V", "SN", "SP"]:
            t = reachable_target(pl["CK"], [t_tag], pl)
            if t and pl["CK"].rp >= 5 and pl[t].buildings:
                deploy(eid, pl["CK"], "saboteur", t, pl, pl[t].buildings[0]["id"], "building")
            if t and pl["CK"].rp >= 3:
    ...
    return finish_game(epoch_id, admin, players, "G1: Fortress vs Blitz",
                       "V=6 guardians (turtle), CK=0 guardians (glass cannon), SN/SP=2 each", TAGS_4)
...
def game_02(token):
    ...
    last = run_foundation(epoch_id, players, admin, {"V": 2, "CK": 2, "SN": 2, "SP": 2})
...
def game_03(token):
    ...
    last = run_foundation(epoch_id, players, admin, {"V": 1, "CK": 1, "SN": 1, "SP": 1}, cycles=2)
...
def game_04(token):
    ...
    epoch_id, players, admin = setup_game(token, "G4: Alliance War 2v2", BASE_CONFIG, TAGS_4,
                                          alliances={"Iron Pact": ("V", "CK"), "Shadow Accord": ("SN", "SP")})
    last = run_foundation(epoch_id, players, admin, {"V": 2, "CK": 2, "SN": 2, "SP": 2})

    def strat(eid, pl, cyc):
        pairs = [("V", ["SN", "SP"]), ("CK", ["SN", "SP"]), ("SN", ["V", "CK"]), ("SP", ["V", "CK"])]
    ...
    return finish_game(epoch_id, admin, players, "G4: Alliance War 2v2",
                       "V+CK vs SN+SP, focused fire on opposing alliance", TAGS_4)
...
def game_05(token):
    ...
    def strat(eid, pl, cyc):
        ...
        if cyc < 12:
            t = reachable_target(pl["SP"], ["V", "CK"], pl)
            if t and pl["SP"].rp >= 4:
...
def game_06(token):
    ...
    def strat(eid, pl, cyc):
        ...
        t = reachable_target(pl["V"], ["CK", "SN", "SP"], pl)
        if t and pl["V"].rp >= 3:
        ...
        t = reachable_target(pl["SN"], ["V", "CK"], pl)
        if t and pl["SN"].rp >= 6:
        ...
        # SP: all offense
        t = reachable_target(pl["SP"], ["V", "CK", "SN"], pl)
        if t and pl["SP"].rp >= 4:
            deploy(eid, pl["SP"], "propagandist", t, pl)
    ...
    return finish_game(epoch_id, admin, players, "G6: Counter-Intel Heavy",
                       "V=CI every 2 cycles, SN=CI every 3 cycles, CK=no CI (all offense)", TAGS_4)
...
def game_07(token):
    ...
    return finish_game(epoch_id, admin, players, "G7: Diplomat Preset",
                       "Diplomatic=30% weight, V+CK alliance, infiltrator-heavy", TAGS_4)
...
def game_08(token):
    ...
    last = run_foundation(epoch_id, players, admin, {"V": 3, "CK": 1, "SN": 2, "SP": 2})
...
def game_09(token):
    ...
    last = run_foundation(epoch_id, players, admin, {"V": 0, "CK": 0, "SN": 0, "SP": 0}, cycles=2)
...
def game_10(token):
    ...
    last = run_foundation(epoch_id, players, admin, {"V": 3, "CK": 3, "SN": 1, "SP": 1}, cycles=4)
    ...
    return finish_game(epoch_id, admin, players, "G10: Builder Preset",
                       "Stability=40%, V+CK=3 guardians (builders), SN+SP=1 guardian (raiders)", TAGS_4)
