...
def game_01(token):
    ...
    epoch_id, players, admin = setup_game(ctx, token, "G1: Fortress vs Blitz", BASE_CONFIG, TAGS_4)
    last = run_foundation(ctx, epoch_id, players, admin, {"V": 6, "GR": 0, "SN": 2, "SP": 2})
    ...
    def strat(eid, pl, cyc):
        ...
        for t_tag in ["V", "SN", "SP"]:
            ...
            if t and pl["GR"].rp >= 5 and pl[t].buildings:
                deploy(ctx, eid, pl["GR"], "saboteur", t, pl, pl[t].buildings[0]["id"], "building")
            if t and pl["GR"].rp >= 3:
    ...
    return finish_game(ctx, epoch_id, admin, players, "G1: Fortress vs Blitz",
...
def game_02(token):
    ...
    ctx = reset_game_state()
    epoch_id, players, admin = setup_game(ctx, token, "G2: Balanced", BASE_CONFIG, TAGS_4)
    last = run_foundation(ctx, epoch_id, players, admin, {"V": 2, "GR": 2, "SN": 2, "SP": 2})
    ...
    run_competition(ctx, epoch_id, players, admin, last+1, 16, strat)
    run_reckoning(ctx, epoch_id, players, admin, 17, 20, strat)
    return finish_game(ctx, epoch_id, admin, players, "G2: Balanced",
...
def game_03(token):
    ...
    epoch_id, players, admin = setup_game(ctx, token, "G3: Warmonger", config, TAGS_4)
    last = run_foundation(ctx, epoch_id, players, admin, {"V": 1, "GR": 1, "SN": 1, "SP": 1}, cycles=2)
    ...
    run_competition(ctx, epoch_id, players, admin, last+1, 16, strat)
    run_reckoning(ctx, epoch_id, players, admin, 17, 20, strat)
    return finish_game(ctx, epoch_id, admin, players, "G3: Warmonger",
...
def game_04(token):
    ...
    ctx = reset_game_state()
    epoch_id, players, admin = setup_game(ctx, token, "G4: Alliance War 2v2", BASE_CONFIG, TAGS_4,
    ...
    run_competition(ctx, epoch_id, players, admin, last+1, 16, strat)
    run_reckoning(ctx, epoch_id, players, admin, 17, 20, strat)
    return finish_game(ctx, epoch_id, admin, players, "G4: Alliance War 2v2",
...
def game_05(token):
    ...
    ctx = reset_game_state()
    epoch_id, players, admin = setup_game(ctx, token, "G5: Betrayal", BASE_CONFIG, TAGS_4,
    ...
    def strat(eid, pl, cyc):
        ...
        if cyc < 12:
            ...
            if t and pl["SP"].rp >= 4:
                deploy(ctx, eid, pl["SP"], "propagandist", t, pl)
        elif cyc == 12:
            log("  *** BETRAYAL: SP attacks ally SN! ***")
            action_log(ctx, cyc, ctx.current_phase, "SP", "BETRAYAL", "SP attacks allied SN!", "TREACHERY")
            t = reachable_target(pl["SP"], ["SN"], pl)
            if t and pl["SP"].rp >= 8 and pl["SN"].agents:
                deploy(ctx, eid, pl["SP"], "assassin", "SN", pl, pl["SN"].agents[0]["id"], "agent")
            if t and pl["SP"].rp >= 5 and pl["SN"].buildings:
                deploy(ctx, eid, pl["SP"], "saboteur", "SN", pl, pl["SN"].buildings[0]["id"], "building")
        else:
    ...
    run_competition(ctx, epoch_id, players, admin, last+1, 16, strat)
    run_reckoning(ctx, epoch_id, players, admin, 17, 20, strat)
    return finish_game(ctx, epoch_id, admin, players, "G5: Betrayal",
...
def game_06(token):
    ...
    ctx = reset_game_state()
    epoch_id, players, admin = setup_game(ctx, token, "G6: Counter-Intel Heavy", BASE_CONFIG, TAGS_4)
    last = run_foundation(ctx, epoch_id, players, admin, {"V": 2, "GR": 2, "SN": 2, "SP": 2})
    ...
    def strat(eid, pl, cyc):
        ...
        if t and pl["SN"].rp >= 6:
            ...
            if emb:
                deploy(ctx, eid, pl["SN"], "infiltrator", t, pl, emb, "embassy")
    ...
    run_competition(ctx, epoch_id, players, admin, last+1, 16, strat)
    run_reckoning(ctx, epoch_id, players, admin, 17, 20, strat)
    return finish_game(ctx, epoch_id, admin, players, "G6: Counter-Intel Heavy",
...
def game_07(token):
    ...
    epoch_id, players, admin = setup_game(ctx, token, "G7: Diplomat Preset", config, TAGS_4,
    ...
    run_competition(ctx, epoch_id, players, admin, last+1, 16, strat)
    run_reckoning(ctx, epoch_id, players, admin, 17, 20, strat)
    return finish_game(ctx, epoch_id, admin, players, "G7: Diplomat Preset",
...
def game_08(token):
    ...
    epoch_id, players, admin = setup_game(ctx, token, "G8: High RP", config, TAGS_4)
    last = run_foundation(ctx, epoch_id, players, admin, {"V": 3, "GR": 1, "SN": 2, "SP": 2})
    ...
    def strat(eid, pl, cyc):
        for tag in TAGS_4:
            ...
            if pl[tag].rp >= 4:
                ...
                if t3:
                    deploy(ctx, eid, pl[tag], "propagandist", t3, pl)

    run_competition(ctx, epoch_id, players, admin, last+1, 16, strat)
    run_reckoning(ctx, epoch_id, players, admin, 17, 20, strat)
    return finish_game(ctx, epoch_id, admin, players, "G8: High RP Economy",
...
def game_09(token):
    ...
    epoch_id, players, admin = setup_game(ctx, token, "G9: No Guardians", config, TAGS_4)
    last = run_foundation(ctx, epoch_id, players, admin, {"V": 0, "GR": 0, "SN": 0, "SP": 0}, cycles=2)
    ...
    run_competition(ctx, epoch_id, players, admin, last+1, 16, strat)
    run_reckoning(ctx, epoch_id, players, admin, 17, 20, strat)
    return finish_game(ctx, epoch_id, admin, players, "G9: No Guardians",
...
def game_10(token):
    ...
    epoch_id, players, admin = setup_game(ctx, token, "G10: Builder Preset", config, TAGS_4)
    last = run_foundation(ctx, epoch_id, players, admin, {"V": 3, "GR": 3, "SN": 1, "SP": 1}, cycles=4)
    ...
    run_competition(ctx, epoch_id, players, admin, last+1, 16, strat)
    run_reckoning(ctx, epoch_id, players, admin, 17, 20, strat)
    return finish_game(ctx, epoch_id, admin, players, "G10: Builder Preset",
