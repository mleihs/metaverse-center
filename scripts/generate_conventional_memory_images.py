"""Generate all 14 images for the Conventional Memory simulation.

7 agent portraits (DOS program screens) + 7 building images (vintage computers)
via the /generate/image API.
...
`supabase start` fresh, IDs will change. This script queries the DB for current IDs.

Usage:
    python3.13 scripts/generate_conventional_memory_images.py
    python3.13 scripts/generate_conventional_memory_images.py --portraits-only
    python3.13 scripts/generate_conventional_memory_images.py --buildings-only
"""
...
def get_entity_ids() -> tuple[list, list]:
    ...
    # Buildings: include type, condition, description, zone for image prompt
...
def main():
    ...
    print("=== Conventional Memory Image Generation ===\n")
    ...
    # Agent portraits (DOS program screens)
    if not buildings_only:
        print(f"--- Agent Portraits ({len(agents)}) ---")
        print("  (These should look like DOS programs on CRT monitors)\n")
    ...
    # Building images (vintage computers)
    if not portraits_only:
        print(f"\n--- Building Images ({len(buildings)}) ---")
        print("  (These should look like vintage DOS-era computers)\n")
