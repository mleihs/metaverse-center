...
def main() -> None:
    ...
    for i, (arch, color) in enumerate(ALL_ARCHETYPES):
        ...
        add(
            f"det_{slug}_{mag}.jpg",
            svc.compose_story_detection(
    ...
    add(
        "det_long_sig_100char.jpg",
        svc.compose_story_detection(
            archetype="The Shadow",
            signature="ANOMALY-SUBSTRATE-RESONANCE-CASCADE-ALPHA-OMEGA-PRIME-ZETA-KAPPA-THETA-EPSILON-0001",
            magnitude=0.95,
            accent_hex="#e74c3c",
        ),
    )
    ...
    add(
        "det_empty_sig.jpg",
        svc.compose_story_detection(archetype="The Tower", signature="X", magnitude=0.01, accent_hex="#f39c12"),
    )
    ...
    add(
        "cls_normal.jpg",
        svc.compose_story_classification(
            archetype="The Tower",
            source_category="Structural Collapse",
            affected_shard_count=3,
            highest_susceptibility_sim="Cite des Dames",
            highest_susceptibility_val=0.88,
            bureau_dispatch="Standard Bureau dispatch. Brief.",
            accent_hex="#f39c12",
        ),
    )
    ...
    add(
        ...
        svc.compose_story_classification(
            ...
            highest_susceptibility_sim="X",
            highest_susceptibility_val=0.1,
    ...
    add(
        "cls_long_dispatch.jpg",
        svc.compose_story_classification(
            archetype="The Shadow",
            source_category="Total Collapse",
            affected_shard_count=15,
            highest_susceptibility_sim="Cite des Dames",
            highest_susceptibility_val=2.5,
            bureau_dispatch=long_dispatch,
            accent_hex="#e74c3c",
        ),
    )
    ...
    add(
        ...
        svc.compose_story_classification(
            ...
            bureau_dispatch="Operatives: report.",
            accent_hex="#e67e22",
        ),
    )
    ...
    for arch, color in [
        ...
        add(
            ...
            svc.compose_story_classification(
                archetype=arch,
                source_category="Standard Resonance",
                affected_shard_count=5,
                highest_susceptibility_sim="Test Shard",
                highest_susceptibility_val=0.75,
                bureau_dispatch="Monitor and report.",
                accent_hex=color,
            ),
        )
    ...
    add(
        ...
        svc.compose_story_impact(
            simulation_name="V",
            effective_magnitude=0.01,
            events_spawned=[],
            narrative_closing="",
            accent_hex="#3498db",
            sim_color_hex="#2980b9",
            banner_bytes=None,
            portraits=None,
            reactions=None,
        ),
    ...
    add(
        ...
        svc.compose_story_impact(
            ...
            effective_magnitude=0.85,
            events_spawned=[
                f"Event {i}: The {['Cathedral', 'Archive', 'Market', 'Border', 'Railway', 'Square', 'Bunker'][i - 1]} Incident"
                for i in range(1, 8)
            ],
            narrative_closing="The Bureau recalibrates.",
            accent_hex="#f39c12",
            sim_color_hex="#d68910",
            banner_bytes=None,
            portraits=None,
            reactions=None,
        ),
    ...
    add(
        "imp_long_closing.jpg",
        svc.compose_story_impact(
            simulation_name="Cite des Dames",
            effective_magnitude=0.72,
    ...
    add(
        ...
        svc.compose_story_impact(
            ...
            accent_hex="#e67e22",
            sim_color_hex="#d35400",
            banner_bytes=None,
            portraits=None,
            reactions=[
                {
                    "agent_name": "Viktor Harken",
                    "text": "This is precisely the sort of disruption that validates our protocols. Increase surveillance in all affected sectors.",
    ...
    for i, (arch, color) in enumerate(ALL_ARCHETYPES[:5]):
        ...
        add(
            f"imp_{slug}.jpg",
            svc.compose_story_impact(
                simulation_name=f"Shard {i + 1}",
                effective_magnitude=mag,
                events_spawned=[f"Event {j}" for j in range(i + 1)],
                narrative_closing="The trembling subsides.",
                accent_hex=color,
                sim_color_hex=color,
                banner_bytes=None,
                portraits=None,
    ...
    add(
        "adv_5v5_max.jpg",
        svc.compose_story_advisory(
            archetype="The Shadow",
            aligned_types=ALL_OPERATIVE_TYPES,
            opposed_types=ALL_OPERATIVE_TYPES,
    ...
    add(
        "adv_empty.jpg",
        svc.compose_story_advisory(
            archetype="The Tower", aligned_types=[], opposed_types=[], zone_name=None, accent_hex="#f39c12"
        ),
    )
    ...
    add(
        ...
        svc.compose_story_advisory(
            archetype="The Deluge",
            aligned_types=["Spy"],
            opposed_types=["Assassin"],
            zone_name="Canal District",
            accent_hex="#3498db",
        ),
    )
    ...
    configs = [
        ...
        (
            "The Deluge",
            ["Infiltrator", "Spy"],
            ["Saboteur", "Propagandist", "Assassin"],
            "The Extended Northern Waterfront Industrial District",
            "#3498db",
        ),
    ]
    for arch, aligned, opposed, zone, color in configs:
        slug = arch.lower().replace("the ", "").replace(" ", "_")
        add(
            f"adv_{slug}_{len(aligned)}v{len(opposed)}.jpg",
            svc.compose_story_advisory(
                archetype=arch, aligned_types=aligned, opposed_types=opposed, zone_name=zone, accent_hex=color
            ),
        )
    ...
    for arch, color in ALL_ARCHETYPES[:5]:
        ...
        add(
            f"sub_{slug}_{ev}ev.jpg",
            svc.compose_story_subsiding(archetype=arch, events_spawned_total=ev, shards_affected=sh, accent_hex=color),
        )
    ...
    add(
        "feed_long_title.jpg",
        svc._compose_with_overlay(
            image_bytes=bg,
            title="SHARD SURVEILLANCE -- The Kanzlerpalast der Administrativen Vollstreckung und Ewigen Ordnung des Staates",
            subtitle="LOCATION: The Democratic Republic of Autonomous Governance and Perpetual Enlightenment",
            color_primary="#e74c3c",
            color_background="#1a0a0a",
    ...
    add(
        ...
        svc._compose_with_overlay(
            ...
            title="PERSONNEL FILE -- Elena Voss",
    ...
    for classif in ("PUBLIC", "AMBER", "RESTRICTED"):
        add(
            f"feed_{classif.lower()}.jpg",
            svc._compose_with_overlay(
                image_bytes=bg,
                title="DISPATCH [0001] -- Classification Test",
