...
|---|---|---|
| J1 | **Zustandsverschmutzung zwischen Tests.** `test_travel_havarie.py::…rueckruf` fällt in etwa jedem dritten VOLLLAUF um, allein nie (3× grün); die Zahl übersprungener Tests schwankt zwischen 2 und 3. Kein Zufall im Test, sondern Zustand, der zwischen Tests überlebt. | Den gemeinsamen Zustand finden (Modul-globale Caches, `supabase_admin_cache`, Instanzspeicher). `pytest -p no:randomly --lf`, dann bisektieren. DRIFT/Paket F. |
