...
- [x] 2.3 Migration 268 travel_sondierung: fn_sondieren, Haul-Split, fn_funkboje_bank + Endpunkte (+ Tests) — 2026-07-13, tests: **20 neu (test_travel_sondierung.py) / 172 travel+drift+pack grün**, ruff + ADR-006-Guard grün. **Zwei Plan-Prämissen hielten der DB nicht stand:** (a) **Der P0-Graph hat KEINE `relais`-Knoten** (verifiziert: 41 interstitial + 7 broadcast_rand, null relais — die kommen erst mit Chart v3 in W3). Die Funkboje wäre bis W3 unbenutzbar gewesen. Jetzt ist das Knoten-Gate eine **Tuning-Liste** (`funkboje_node_types` = `["relais","broadcast_rand"]`): sie funktioniert heute an den Broadcast-Kanten der Welten — die bessere Fiktion ohnehin (man sichert Haul, indem man ihn von einer Welt mit Empfänger SENDET) — und `relais` tritt in W3 ohne Codeänderung dazu. (b) **`haul_banked` war schon vergeben:** `fn_travel_bank_run` (265) schreibt es als ABSCHLUSS-QUITTUNG. Der Plan wollte denselben Namen für die In-Flight-Reserve → ein Schlüssel, zwei Bedeutungen, und der Bug wäre still gewesen (die Quittung entsteht erst beim Schließen, also hätte die Carry-Liste nichts getragen und jede gesicherte Vermessung wäre beim nächsten Zug verdampft). Reserve heißt jetzt `haul_safe`; die Carry-Whitelist aus 267 hatte den falschen Namen vorregistriert und ist korrigiert. **Weitere Entscheidungen:** (c) **Der Bust nimmt NUR den losen Ertrag DIESES Knotens** — nie die Reserve, nie das Manifest, nie den Run. Genau diese Asymmetrie macht die Funkboje zur Entscheidung statt zur Formalität. (d) **Eine Zerfaserung ZAHLT die Reserve** (`fn_travel_zerfasern`): was der Träger gesendet hat, kommt an, auch wenn er selbst es nicht tut. Ohne diese Zeile wäre Banken nur eine Absicherung gegen den Rückruf-Multiplikator — eine Rundungsentscheidung, kein Wagnis; das ganze Push-your-luck fiele in Arithmetik zusammen. (e) **Ein Störungs-Marker (`marker_add`, 267) zählt in den Stapel** — der Drift kann eine Grabungsstelle vergiften; der Stapel, der platzt, ist nicht immer der selbstgebaute. (f) **Banken zu Hause wird abgelehnt** (`AT_HOME`): dort zahlt die Entladung 100 %, ein Bank-Klick könnte nur Geld kosten (dritte Anwendung der W1-Regel „eine tote Option ist schlimmer als keine"). (g) Ein rissiger Knoten hebt das Störungs-Gewicht im Draw (`sondierung_rissig_stoerung_bonus`) — der Draw liest die Flagge selbst aus dem Checkpoint, deshalb musste `fn_travel_move` NICHT erneut angefasst werden. (h) Graben IST Reisen: es kostet einen Takt aus demselben Fenster, und der letzte Takt fern von zuhause strandet den Run genau wie ein Zug. (i) **Regress unterwegs gefangen:** `haul_transmitted` lag zuerst unbedingt im Abschluss-Checkpoint und brach die Gate-off-Byte-Parität (der W1-Ökonomie-Test pinnt den Schlüsselsatz exakt — genau dafür). Jetzt hinter dem Gate, in Abschluss UND Wrack: Gate zu = null Fun-Kern-Residuum.
- [x] 2.4 FE: Signal-Panel-Ausbau, DriftMarkerStack, DriftLogbook, Sondier-/Riss-Zeremonien (M-05..M-11) + Endpunkt-/Typen-Anbindung (+ E2E) — 2026-07-13, `npm run lint:full` grün (tsc/biome/7 Gates), 173 Backend-Tests grün, **browser-verifiziert**: Grabung (Ertrag 2→3→5→8→8 exakt aus dem Server-Tuning, offener Oktagon-Marker-Stapel) → **Resonanzriss** (dritter Statik-Marker, 34 Punkte bleiben im Knoten, DZ +6, Knoten rissig, HUD + Toast sagen es beide) → passives Gerücht (nur Log-Zeile, kein Modal) → **Störung** (Panel hält den Run an) → **Begegnung** (blauer Rahmen statt Gefahr-Rot; Chips „1 Takt" vs. „Riskant · Sprache") → Antwort → Ergebnis-Panel mit Delta-Bilanz (Dissonanz +3, Takte −1, deckungsgleich mit dem HUD) → Logbuch → **Funkboje** (Knopf verspricht „14 von 20", danach „0 lose, 14 gesendet und sicher"). **Entscheidungen:** (a) Das W1-Storylet-Panel trägt die Signale ohne Formänderung — nur ein `scene-kind`-Attribut für den KLASSENSPEZIFISCHEN EINTRITT (M-05 Scanline-Wipe + Chroma-Jitter für die Störung, M-06 weiches Einschweben für die Begegnung) und eine Delta-Bilanz im Resultat. Verschieden ANKOMMEN, identisch ENTSCHEIDEN. (b) **Die Odds werden nie beziffert** (R4): der Chip sagt „Riskant · Architektur" und nennt den Vektor, die Schwierigkeit bleibt beim Bureau. Die KOSTEN dagegen stehen exakt drauf — sie werden vorab gezahlt, egal wie der Wurf ausgeht, und ein Chip, der nur im Erfolgsfall gilt, wäre eine Lüge. (c) **Der Marker-Stapel ist die ganze Mechanik**: Beweise zählbar, Risiko unbeziffert. Zwei gleiche Marker pulsen VOR dem Commit, nicht danach. (d) `sondierung_yields` / `funkboje_rate` / `funkboje_node_types` wandern in die Tuning-Response — der nächste Ertrag ist eine Server-Zahl, keine Client-Konstante (sonst macht ein Re-Tuning das Panel zum Lügner). (e) Der Funkboje-Knopf erscheint **nicht zu Hause** — der Server lehnt es ab (`AT_HOME`), und was der Server ablehnt, gehört nicht aufs Brett. (f) `vectorLabel` wird aus `DriftChartHost` exportiert statt dupliziert (Brett und Panel müssen dieselben Vektoren gleich nennen). (g) Escape schließt Bericht und Ergebnis, **nicht** die Störung — ein angehaltener Run ist nicht wegdrückbar (W1-Regel, jetzt auch für Signale). **Zwei Bugs, die erst der Playtest zeigte:** (1) **`fn_signal_resolve` schrieb `last_signal.class`, das Modell liest `signal_class`** → JEDER GET auf den Run warf 500 (das Modell validiert die ganze Zeile, nicht nur die Antwort der Mutation), während der RPC selbst perfekt grün war. Die 172 Tests lasen die rohe Run-Zeile; nur das Modell sieht diesen Vertrag. Fix an der Quelle + Regressionstest, der den aufgelösten Run durch `TravelRunResponse` schickt. (2) **Zwei Migrationen besaßen `drift_checkpoint_carry`** (267 legte sie an, 268 korrigierte den Schlüsselnamen) — ein erneutes Anwenden von 267 machte 268 still rückgängig (drei Tests wurden exakt so rot). Jetzt trägt 267 den richtigen Schlüssel und 268 fasst die Funktion nicht mehr an: **eine Funktion, eine Migration.**
