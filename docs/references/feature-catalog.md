...
| S60 | **BYOK (Bring Your Own Key)** | ✅ IMPL | User können eigene OpenRouter/Replicate API-Keys hinterlegen (AES-256 verschlüsselt via `user_wallets`). Forge nutzt User-Keys wenn vorhanden, sonst Platform-Keys. Standalone `<velg-byok-panel>` Komponente (user/admin Mode). Key-Test gegen Provider-API vor Speicherung (`POST /wallet/keys/test`). Einzelne Keys löschbar (`DELETE /wallet/keys/{provider}`). Format-Validierung (sk-or-/r8_ Prefix), Reveal-Toggle, Revoke mit Bestätigungsdialog. Migration 218 erweitert RPC um `p_clear_*` Flags. |
...
| RD2 | **Combat System** | ✅ IMPL | Shared `backend/services/combat/` Modul (6 Dateien). Phase-basierter Kampf: Planning (60s Timer, remaining_ms clock-skew-frei) → Resolution → Outcome. 18 Fähigkeiten in 6 Schulen (Spy/Guardian/Assassin/Propagandist/Saboteur/Infiltrator). Stalemate-Mechanik bei Runde 10. |
...
| RD6 | **Dungeon Frontend** | ✅ IMPL | DungeonTerminalView (CRT/Submarine-Ästhetik), DungeonHeader (Depth Gauge + Visibility Pips + Stability Gauge), DungeonMap (SVG mit reveal-blip/depth-ping Micro-Animationen), DungeonCombatBar (60s Timer mit remaining_ms clock-skew-frei, 3-Stufen-Urgency: amber→warning→critical, Onboarding-Grace +10s, multi-layer Danger-Animation bei ≤5s, kompaktes 2-Spalten Onboarding-Briefing), DungeonPartyPanel (Aptitude-Tooltips), DungeonEnemyPanel. 5 semantische Combat-Log-Farben. Terminal-Command-Event-Forwarding. BureauTerminal-Statusbar mit `LOC:`/`RUN:` Kontext-Prefix. Bare-Number-Shortcuts (z.B. "3" → contextual interact/move). Instance-TTL-Cleanup (60s Loop, evict + DB-RPC `fn_expire_abandoned_dungeon_runs`). |
...
| RD18 | **Deluge: Elemental Warding** | ✅ IMPL (seit Migr. 289 erreichbar) | Der **Leser** steht seit Migration 174 im Event Service: aktive Warding-Effekte schützen Buildings vor Degradation (`created_at + duration_ticks * heartbeat_interval`, expired → consumed). Der **Schreiber** fehlte: Migration 174 setzte den `simulation_modifier`-Zweig in die 6-arg-Überladung von `fn_apply_dungeon_loot`, die nie ein Aufrufer hatte — die laufende 3-arg-Fassung kannte den Typ nicht, also entstand nie eine Zeile. Migration 289 zieht den Zweig in den laufenden Pfad. Bis dahin war das Merkmal vollständig gebaut und nicht auslösbar. |
...
| RD27 | **The Awakening (7. Archetyp)** | ✅ IMPL | Awareness-Gauge 0-100 (Bewusstseinsdrift). 5 Feinde (Echo Fragment, Deja-vu Phantom, Consciousness Leech, Repressed Sentinel, The Repressed Boss). 17 Encounters (6C+5N+1E+1B+2R+2T). 28 Banter (awareness_tier 0-3, literarisches Register: Kafka-Proust-Lem-Stapledon). 11 Loot (T3: Awakening Insight = Big Five ±0.1). 8 Objektanker (Spiegelscherbe, Philemons Feder, Madeleine, Zwei Uhren, Hohlenschatten, Empathiebox, Einhornschadel, Turhuter). Ground-Action (Spy, -10 awareness, 3-Room CD). Deja-vu Room Morphing (cleared rooms reconstruct at awareness ≥50). Lucid Dreaming Bonus (+10% aptitude in combat/skill checks at awareness ≥70). Boss Deployment (3 introspection choices). Frontend: Awareness Gauge (violet-cyan-white), Awakening Protocol Briefing, Barometer. Migration 176 (84 Content Items) + Migration 177 (personality_modifier RPC). |
...
| K8 | **Content Drafts Admin (A1.7)** | ✅ IMPL | Platform-weite YAML-Pack-Edits via Admin-Surface statt direkter Git-Commits. `<velg-admin-content-drafts-tab>` orchestriert 5 Modals: List (paginiert, Status-Filter, PR-Grouping), Editor (CodeMirror 6 JSON + 3-Way-Conflict-View), Publish-Batch (1-25 Drafts → 1 GitHub-PR via GitHub App), Sweep-Orphans (manuelle Classification + Delete-Preview), Scheduler-Settings (K9). Status-Maschine: draft → conflict → published → merged | abandoned mit DB-Trigger-Enforcement (Migration 226). 3-Way-Merge-Service mit recursive dict-field-merging + entry-gruppierter Conflict-UI. Optimistic concurrency via `version`. Webhook-Handler revertiert published→draft bei PR-close. Author-Email-Resolution via `get_user_emails_batch`. 8 Phasen-Implementierung mit 30+ Audit-Findings über 6 Passes. |
| K9 | **Orphan-Sweeper Scheduler (A1.7 Phase 7b+7c)** | ✅ IMPL | Wöchentlicher Scheduled-GC für abgebrochene `content/drafts-batch-*` Branches. Backend: `OrphanSweeperScheduler` via `BaseSchedulerMixin` mit `last_run_at` Throttle in `platform_settings` (Migration 227, 4 Keys), Sentry auf delete-Failures mit `trigger=scheduled|admin` Tag. `run_sweep_and_persist(admin, *, now, config, trigger)` public classmethod geteilt zwischen Tick-Loop und Admin-Endpoint. Admin: `<velg-orphan-sweeper-settings-modal>` (Gear-Button in Content-Drafts-Liste) mit Enable-Toggle (auto-save + optimistic rollback), Interval/Min-Age Number-Inputs (600ms debounce + blur-commit), last-run Readout mit relative-time chip, Run-now Button mit ConfirmDialog + optimistic-stamp. `POST /api/v1/admin/content-drafts/orphan-sweeper/run-now` Endpoint bypasst Throttle, audit-logged als `orphan_sweeper_run_now`. 6 Audit-Passes (F76 timer-leak, F77 CSS-transition, F80 ISO-format-drift, F81 placeholder, F82-F85). |
