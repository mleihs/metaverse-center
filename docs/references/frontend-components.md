...
| settings/ | 13 | 13 | SettingsView + 12 panels (General, World, Bleed, Autonomy, Bonds, Weather, AI, Prompts, Integration, Design, Features, Access, Notifications) |
...
| shared/ | 34 | 23 | 23 components + 11 CSS modules + 1 base class. terminal-theme-styles.ts exports: terminalTokens, terminalComponentTokens, terminalAnimations, terminalFormStyles, terminalOAuthStyles, terminalFrameStyles, terminalWrapperStyles, terminalActionStyles. title-group-styles.ts exports: titleGroupStyles (shared `.title-group` inline-flex wrapper paired with `<velg-help-tip>`). VelgHelpTip: `?` icon that SPA-navigates to `/how-to-play/guide/<TopicSlug>` — threaded into 10 views (agents/buildings/events/bonds/locations/health/social + forge mint/BYOK panel/integration settings) |
...
├── icons.ts                        # Centralized SVG icons with aria-hidden="true" (~106 icons). Includes dungeon set: dungeonDepth, doorOpen, diamond/diamondEmpty, binoculars, doorExit/doorEnter, campfire, handClick, treasure, crown, shield, dagger, mask, bomb, footprints, dungeonMap, skullBolt, questionCircle, volume, volumeOff. Chat set: copy, thumbsUp, thumbsDown, refresh, bookmark, link, smile, pin, userPlus
...
├── dungeon-formatters.ts           # 20 pure formatters: formatDungeonMap, formatRoomEntry (incl. Deluge water bar + threshold), formatArchetypeBriefing (all 8 archetypes), formatCombatStart, formatCombatResolution (bilingual narrative), formatRoundTransition, formatEncounterChoices, formatThresholdEntry, formatLootDrop, formatSkillCheckResult, formatPartyWipe, formatDungeonComplete, formatSealResult, formatSalvageResult, formatDebrisFound, etc.
