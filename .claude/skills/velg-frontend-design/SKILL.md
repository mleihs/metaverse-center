...
6. **No layout container effects**: Never apply `filter`, `transform`, `will-change`, `contain: paint`, or `perspective` on shells, views, or panels. These create containing blocks that break `position: fixed` modals. Apply to leaf elements only.
