---
name: velg-frontend-design
description: Create distinctive Lit 3 web components for metaverse.center using the project's brutalist design system, 3-tier color tokens, shared components, and strict linting gates. Use this skill before writing any frontend component code.
---

This skill guides creation of production-grade Lit 3 web components for the metaverse.center platform. Every component must follow the project's brutalist dark-theme design system, reuse shared components, and pass all lint gates. The aesthetic is "Cold War intelligence dossier meets simulation engine" — typewriter headings, amber accents on near-black surfaces, hard offset shadows, no rounded corners by default.

The user provides frontend requirements: a component, view, panel, or interface to build. They may include context about the simulation domain, audience, or technical constraints.
...
export class VelgMyComponent extends LitElement {
  static styles = [
    buttonStyles, // Compose shared style modules first
    css`
      :host {
        /* Tier 3: component-local tokens from Tier 1/2 */
  ...
  // Three-state render pattern (MANDATORY)
  protected render() {
...
5. **No layout container effects**: Never apply `filter`, `transform`, `will-change`, `contain: paint`, or `perspective` on shells, views, or panels. These create containing blocks that break `position: fixed` modals. Apply to leaf elements only.
...
Remember: Claude is capable of extraordinary creative work within constraints. The brutalist design system is not a limitation — it's a vocabulary. Use it to create interfaces that feel like declassified intelligence briefings, war room dashboards, and ancient terminal readouts. Don't hold back — show what can be created when committing fully to a distinctive vision within a disciplined system.
