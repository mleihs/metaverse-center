...
- Never hand-wire a Supabase query-chain mock in a test. Use `make_chain_mock()` / `make_table_mock()` / `make_async_supabase_mock()` from `backend/tests/conftest.py`. A local `builder.select.return_value = builder` list drifts the moment a service starts calling a method it does not name, and MagicMock does not complain — it invents the attribute, forks the chain, and the test then either dies on `await` with a TypeError naming neither method nor query, or keeps asserting against a builder the code never touched. The one list lives in `conftest.CHAIN_METHODS` and is pinned against the production call sites by `scripts/lint-chain-mock-covers-postgrest.sh` (CI `lint-*` job). Same rule for `app.dependency_overrides[get_admin_supabase]`: it must be able to answer the `platform_admins` refresh, because `reset_platform_admin_cache()` now runs between tests — a bare `MagicMock()` only ever worked on a cache that another test file had warmed.
...
- Never write an SVG fragment with lit's `html` tag. Use `svg` from `lit`. A nested ``html`` `` template is parsed as HTML even when it renders inside an `<svg>`, so the browser builds the element in the XHTML namespace — and an SVG shape in the XHTML namespace draws NOTHING, with no error, no warning and no console output. Measured (lit 3.3.3, one render pass): ``html`<line/>` `` → `ns=http://www.w3.org/1999/xhtml` / `HTMLUnknownElement`; ``svg`<circle/>` `` → `SVGCircleElement`; a `<path/>` written directly in the surrounding template → `SVGPathElement`. The agent-needs radar in `AgentMoodPanel` drew its grid rings, axes and data points through `.map(… => html`…`)` and every one of them was invisible from the day it shipped; only the filled area rendered, because that one was written inline. Enforced by `frontend/scripts/lint-svg-fragments-use-svg-tag.sh` in CI. `lit-analyzer` catches only part of it (`no-unclosed-tag` fires for `<line>`/`<circle>` but not `<path>`).
- Never leave a glob unquoted in an npm script. `"lint:lit": "lit-analyzer src/**/*.ts"` ran through `sh`, which has no `globstar`, so `**` collapsed to `*` and the gate saw **60 of 524 files** — it reported `0 problems` and exited 0 for months while 124 findings sat behind it, including a `<button>` nested inside a `<button>` and the invisible radar above. Quote the pattern so the tool does its own expansion.
