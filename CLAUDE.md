...
backend/              FastAPI application
  app.py              Entry point (registers 32 routers)
  ...
  routers/            API endpoints — 32 routers, 244 endpoints (/api/v1/... + /api/v1/public/... + /api/v1/admin/...)
  models/             Pydantic request/response models (20 files)
  services/           Business logic (BaseService + 27 entity + audit + simulation + external + email + admin + bot)
  ...
  tests/              pytest tests (499 tests: unit + integration + security + performance)
frontend/             Lit + Vite application
  src/
    ...
    components/
      ...
      multiverse/   CartographerMap, MapGraph, MapConnectionPanel, MapTooltip, MapBattleFeed, MapLeaderboardPanel, MapMinimap, map-force, map-data, map-types
      ...
      admin/          AdminPanel (tabbed shell), AdminUsersTab (user/membership CRUD), AdminCachingTab (cache TTL controls)
      health/         SimulationHealthView
    services/         Supabase client, 21 API services (incl. AdminApiService, BotApiService), AppStateManager, NotificationService, ThemeService, theme-presets, SeoService, AnalyticsService
...
supabase/
  migrations/         43 SQL migration files (001-037 + ensure_dev_user + 040 platform_settings + 041 bot_players + 042 rls_hardening + 043 cite_des_dames)
  seed/               18 SQL seed files (7 active: 001, 006-008, 010, 015-016; 11 archived with _ prefix: 002-005, 009, 011-014, 017-018)
  config.toml         Local Supabase config
scripts/              Image generation scripts (6: velgarien, capybara, station_null, speranza, cite_des_dames, dashboard) + lore images + epoch simulation
docs/                 Spec documents (28 files: 00-23 + GAME_DESIGN_DOCUMENT + concept + SIMULATION_BLUEPRINT)
```
...
- **140+ RLS policies** — CRUD per table, role-based via helper functions + 21 anon SELECT policies for public read access + embassy policies + competitive layer policies + epoch chat policies + bot player policies
...
- **19 functions** — RLS helpers, utility (generate_slug, validate_taxonomy_value), trigger functions, admin RPC (admin_list_users, admin_get_user, admin_delete_user via SECURITY DEFINER)
...
- **Role checking:** `require_role("admin")` factory returns Depends() that checks simulation_members. `require_epoch_creator()` checks epoch ownership. `require_simulation_member("editor")` checks role via query param (for platform-level endpoints). `require_platform_admin()` checks email allowlist (`admin@velgarien.dev`) for platform admin endpoints — uses `get_admin_supabase()` (service_role) for DB access.
...
- **Admin services:** `AdminUserService` (user listing/detail/deletion via SECURITY DEFINER RPC, membership CRUD via PostgREST), `PlatformSettingsService` (key-value settings CRUD with cache invalidation), `CacheConfigService` (singleton reading TTL values from `platform_settings` table, used by echo_service, seo middleware, public router HTTP Cache-Control headers)
...
28 specification documents in `docs/` (00-23 + GAME_DESIGN_DOCUMENT + concept + SIMULATION_BLUEPRINT). **Always consult the relevant spec before implementing:**

| Doc | Content | Version |
|-----|---------|---------|
| `docs/03_DATABASE_SCHEMA_NEW.md` | Complete schema (44 tables, triggers, views, RLS) | v2.7 |
| `docs/04_DOMAIN_MODELS.md` | TypeScript interfaces (aligned with schema v2.7) | v2.7 |
| `docs/05_API_SPECIFICATION.md` | 244 endpoints with request/response formats | v1.7 |
| `docs/07_FRONTEND_COMPONENTS.md` | Component hierarchy, routing, state management | v1.9 |
| `docs/09_AI_INTEGRATION.md` | AI pipelines, prompt system, model fallback | v1.0 |
| `docs/10_AUTH_AND_SECURITY.md` | Hybrid auth, JWT validation, RLS strategies | v1.0 (rewritten) |
| `docs/12_DESIGN_SYSTEM.md` | CSS tokens, brutalist aesthetic, component styles | v1.0 |
| `docs/13_TECHSTACK_RECOMMENDATION.md` | All config templates (pyproject, package.json, etc.) | v1.3 |
| `docs/17_IMPLEMENTATION_PLAN.md` | 160 tasks, 6 phases, dependency graph | v2.9 |
| `docs/18_THEMING_SYSTEM.md` | Per-simulation theming: token taxonomy, presets, ThemeService, contrast rules | v1.1 |
| `docs/19_DEPLOYMENT_INFRASTRUCTURE.md` | Production deployment, dev→prod sync, data migration playbook | v1.0 |
| `docs/20_RELATIONSHIPS_ECHOES_MAP.md` | Agent relationships, event echoes, cartographer's map — feature docs + user manual | v1.0 |
| `docs/21_EMBASSIES.md` | Embassies & ambassadors: cross-sim diplomatic feature, visual effects, user manual | v1.0 |
| `docs/22A_GAME_SYSTEMS.md` | Game mechanics: materialized views, computed attributes, bleed system | v1.0 |
| `docs/22B_EPOCHS_COMPETITIVE_LAYER.md` | Competitive PvP: epochs, operatives, scoring, battle log, epoch chat, realtime | v1.2 |
| `docs/23_MICROANIMATIONS.md` | Dramatic entrance animations, stagger patterns, theme-aware motion across all UI | v1.0 |
| `docs/GAME_DESIGN_DOCUMENT.md` | High-level game design concept and vision | v1.0 |
