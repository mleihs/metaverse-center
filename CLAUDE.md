...
- Supabase client hierarchy: `get_supabase` (user JWT, RLS enforced) → `get_effective_supabase` (auto-elevates to service_role for platform admins) → `get_admin_supabase` (service_role, bypasses all RLS). Every router that touches simulation data uses `get_effective_supabase` (45 of 59 routers; the rest are public/anon or service-only surfaces). One documented exception: `drift.py` writes through `get_supabase` (user JWT) on purpose — the DRIFT run-state RLS uses `auth.uid()` as the identity and admin auto-elevation would break run ownership (migration 246 §4); its reads and the gate check follow the normal pattern.
...
- Never use `response_model=` on FastAPI decorators. Use return type annotations instead (`-> SuccessResponse[T]`). The codebase has zero `response_model=` usage on API endpoints — removed in the Pydantic response typing refactor (468 endpoints, 46 routers, 45 models). Sole benign exception: the SPA catch-all `serve_spa` in `app.py` uses `response_model=None` because its `FileResponse | HTMLResponse | RedirectResponse` union is not a Pydantic model.
...
- Never let a migration's self-check assert against the platform's CONTENT — only against its own EFFECT. A `RAISE EXCEPTION` on a row count (`IF v_rows <> 40`), on the existence of a particular world (`IF v_sim IS NULL`), or on data a *later* step provides (the seed runs AFTER migrations) passes on production by coincidence and can never pass on a fresh database. Five migrations carried this and CI was red for two days because of it (299 UPDATE before the seed · 305 `<> 40` / `<> 36` as prod row counts · 309 needs a building · 311 needs a world with `pristine` · 314 needs a completed draft). Structural assertions are the right kind and stay: does the function exist, does the view have N columns, are there N RLS policies, is the index there, is `anon` denied. Where a probe genuinely needs data, capture the BEFORE state in a `TEMP TABLE` and compare after (305), or guard the precondition with `RAISE NOTICE` and skip — never silently, because an unrun test is not a passed one. A migration that cannot run on an empty database is not verified, it is unverifiable. And the other side of the same coin: **a check that passes because it found nothing to check is not a passed check** — when a probe has to skip for lack of data, say so with `RAISE NOTICE`, never in silence.
...
- Never grant SECURITY DEFINER functions to `anon` or `authenticated`. PostgREST exposes every EXECUTE-granted function at `/rest/v1/rpc/<fn>` and a SECURITY DEFINER fn runs as its owner, so an anon/authenticated grant lets a caller bypass the FastAPI `Depends()` role gate. Privileged RPCs must be callable only via the backend service-role client (`get_admin_supabase_client()`); the router validates authorization first (see ADR-006, incident migration 096→147, sweep migrations 257+258). Switching a call site from the user-JWT client to the admin client for the `.rpc()` is behaviorally neutral (SECURITY DEFINER already runs as owner) — keep surrounding RLS-enforced `.table()` ops on the user client. Two narrow exceptions stay `authenticated`-callable: (a) functions that self-validate the caller (`auth.uid() IS DISTINCT FROM <target> → RAISE`, or use `auth.uid()` as the identity with no user-id param — e.g. the DRIFT travel/quest RPCs, `fn_purchase_tokens`); (b) genuine public reads. Enforcement: the anon surface is pinned by `scripts/lint-no-secdef-public-grant.sh` (CI `test-backend` job, post-migration DB-state check, allowlist = `get_bleed_status` + 4 RLS helpers) — add to its allowlist only with a proven justification. Note: `ALTER DEFAULT PRIVILEGES … REVOKE EXECUTE … FROM PUBLIC` does NOT close this on Supabase — `pg_default_acl` grants EXECUTE directly to `anon`/`authenticated` (for both `postgres` and the managed `supabase_admin` role), so the grant must be revoked per-function in a migration.
- Never apply a NARROWING migration before the code is deployed that copes without what it removes. Narrowing: `REVOKE`, `DROP POLICY`, `DROP COLUMN`, a new `NOT NULL`, a tightened `CHECK`, a narrowed FK. Widening (`GRANT`, a nullable column, a looser `CHECK`) is harmless in either order — it only widens what the running code already had. There is no automatism that ships migration and code together: CI applies migrations only to a throwaway local Supabase, the Dockerfile has no migration step and there is no deploy hook, so whoever migrates by hand stands, for a while, in a state no commit describes. The DIRECTION of that window decides. So: widening BEFORE the deploy, narrowing AFTER — and *after* means verified, not assumed: `LIVE=$(curl -s https://metaverse.center/ | grep -oE 'velg-release" content="[0-9a-f]+' | grep -oE '[0-9a-f]{40}')` then `git merge-base --is-ancestor <fix-commit> $LIVE && echo "code is out"` (the page carries `<meta name="velg-release">`; no SSH and no token needed). On 2026-09-05 migration 387 revoked EXECUTE on three BYOK functions from `authenticated` before `a10d8934` shipped: the Forge answered 503 on the wallet and 500 on every BYOK endpoint, for every non-admin user. **It was invisible to the operator, because `get_effective_supabase` elevates platform admins to `service_role` — the failure hid from exactly the person who would have noticed it.** And checking the grant is not checking the feature: permissions answer *may it*, a real call answers *does it*.
- Never write a prompt template that names a placeholder the rendering call site does not supply, and never write one in Mustache syntax. `prompt_templates` is data; `PromptResolver` renders it `str.format` style, so `{name}` is the only placeholder form and every other brace is literal (which is why a JSON example inside a prompt needs no escaping). The variables each `template_type` may use are declared once in `backend/services/prompt_contracts.py`, together with the platform `frame` — the composition, subject count and output-format guarantees that `PromptResolver` appends to any *simulation-owned* template and that a world therefore cannot edit away. Three consumers read that declaration and nothing else: the resolver (renders, reports undeclared placeholders to log + Sentry), `ForgeThemeService` (builds the A.6 generation prompt from it, sanitises the model's output before storing, writes `variables` as a real JSON array), and `PromptTemplateService` (rejects a hand-written violation with the list of allowed names). `backend/tests/unit/test_prompt_contracts.py` binds the declaration to the call sites by AST, so a variable added in a service and not in the contract is a red test. A CHECK constraint (migration 280) refuses `{{name}}` at the database. Adding a template type means adding its contract; without one the type is simply unmanaged, and the repair script leaves it alone. See `docs/plans/forge-w1-prompt-template-contract.md`.
...
- Never use `get_supabase` directly in routers. Use `get_effective_supabase` instead — it auto-elevates to service_role for platform admins, returns the user-scoped client for everyone else. Without this, admins pass `require_role()` but fail on RLS. Single documented exception: `drift.py` (run-state RLS keyed on `auth.uid()`, see its module docstring — admin elevation would break run ownership there).
...
- Never UPDATE/INSERT/DELETE per-simulation map-geometry tables (`zones.geojson`, `city_streets`, `buildings.geojson`, `cities.map_center_lat/lng`, or `building_agent_relations(relation_type='lives_at')`) directly from Python. All geometry persistence goes through `fn_apply_map_geometry` (atomic SQL function, migration 236). Generation is Python (shapely in `ForgeMapService` — geometry-only logic that has no SQL equivalent), but every write happens inside one SQL function that bumps `simulations.map_geometry_version` and sets `forge_drafts.map_status='succeeded'` as the last statement — failure → rollback. See ADR-007 (Postgres-first principle) and `docs/plans/per-simulation-world-map-plan.md`.
...
- Never place `bureauPanelFrameStyles` (from `components/shared/bureau-panel-styles.ts`) anywhere other than the LAST element of a panel's `static styles = [css` … `, bureauPanelFrameStyles]` array. The shared module declares `:host { background: ... }` and only wins the cascade vs the panel's own `:host` background rule by virtue of coming last. Reversing the order silently kills the Bureau-Dispatch frame (no TS/CSS error, just an invisible regression). Enforced by `frontend/scripts/lint-bureau-panel-frame-last.sh` in CI.
- Never put MapLibre or other canvas-heavy WebGL components inside Shadow DOM. Override `createRenderRoot() { return this; }` to use the light DOM as the render root. Shadow DOM has documented event-coordinate quirks with MapLibre (canvas pointer-event coordinates resolve to the wrong element, see MapLibre discussion #1304). New per-simulation world-map components go in `frontend/src/components/world-map/`, NOT `frontend/src/components/map/` — that directory is the multiverse schematic (`CartographicMap`, `MapAnnotationTool`), a different feature; do not extend it.
- For light-DOM-render-root components nested inside a Shadow-DOM hierarchy (e.g. `<velg-simulation-world-map>` lives inside `velg-app`'s shadow root): `document.head` is NOT a usable global CSS scope. `<style>` elements there will NOT pierce into ancestor shadow roots — the component renders with default `display: inline`, layout breaks, and third-party CSS (e.g. MapLibre's `.maplibregl-ctrl-top-right { position: absolute }`) silently does nothing. Resolve the containing scope via `this.getRootNode()` and inject the `<style>` there (ShadowRoot or Document, with idempotency via a `WeakSet`). Get the CSS as a STRING via Vite's `?inline` query (instead of Vite's default which would auto-inject into `document.head`) — works identically for own and third-party CSS: `import COMPONENT_CSS from './component.css?inline'` (static, for own component CSS) or `const mod = await import('lib/dist/lib.css?inline'); appendStyle(scope, mod.default)` (dynamic, for code-split third-party CSS). Canonical reference: `frontend/src/components/world-map/SimulationWorldMap.ts` (`ensureComponentStyles` + `ensureMapLibreCss`) plus `frontend/src/components/world-map/world-map.css`.
- For flex layouts containing MapLibre or other elements with internal `ResizeObserver`s: `min-height` alone does NOT bound the layout — flex items default to `min-height: auto` (= "as tall as content"), and MapLibre's canvas grows unboundedly (observed: 23000px tall canvas before fix). Use explicit `height` (e.g. `clamp(540px, calc(100vh - 240px), 880px)`) on the outer flex container, plus `min-height: 0` on all flex children in the canvas chain, plus `position: absolute; inset: 0` on the MapLibre target inside a `position: relative` wrapper.
