...
- Never ship a migration with multiple `CREATE OR REPLACE FUNCTION` blocks separated only by SQL comments (`--`). The Supabase CLI's migration splitter collapses such files into a single prepared statement and fails with `SQLSTATE 42601 "cannot insert multiple commands into a prepared statement"`. Every function in a multi-function migration must be followed by at least one non-comment statement — canonical choice is `COMMENT ON FUNCTION public.fn_name(arg_types) IS '...';` (which also enforces documentation). Reference pattern: migration 148 (7 functions, each with COMMENT), migration 197 (9 functions, each with GRANT EXECUTE + COMMENT), migration 214 (12 functions with COMMENT). Enforced by convention; a lint gate would require a PG-aware SQL parser.
