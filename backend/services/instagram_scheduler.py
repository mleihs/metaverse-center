...
class InstagramScheduler(BaseSchedulerMixin):
    ...
    async def _process_due_posts(cls, admin: Client, config: dict) -> None:
        ...
        logger.info(
            "Found due Instagram posts to publish",
            extra={
                "post_count": len(due),
        ...
        for post in due:
            ...
            try:
                ...
                if retry_count < MAX_PUBLISH_RETRIES:
                    ...
                    logger.warning(
                        "Container error for post, resetting to scheduled",
                        extra={
                            "post_id": str(post_id),
                            "retry_count": retry_count + 1,
                ...
                logger.exception(
                    "Unexpected error publishing post",
                    extra={
                        "post_id": str(post_id),
                        "iteration": cls._iteration_count,
                    },
    ...
    async def _publish_single_post(
        ...
        if media_id:
            try:
                ...
                logger.warning(
                    "Failed to fetch permalink for media",
                    extra={
                        "post_id": post_id,
                        "media_id": media_id,
                    },
    ...
    async def _compose_pending_stories(cls, admin: Client) -> None:
        ...
        for story in pending:
            ...
            try:
                ...
                if url:
                    logger.debug(
                        "Composed story image ahead of schedule",
                        extra={
    ...
    async def _process_due_stories(cls, admin: Client, config: dict) -> None:
        ...
        logger.info(
            "Found due Instagram stories to publish",
            extra={
                "story_count": len(due),
                "iteration": cls._iteration_count,
                "config_enabled": config["posting_enabled"],
        ...
        for story in due:
            ...
            try:
                ...
                if retry_count < MAX_PUBLISH_RETRIES:
                    ...
                    logger.error(
                        "Story failed after max retries",
                        extra={
                            "story_id": story_id,
                            "iteration": cls._iteration_count,
                        },
                ...
                logger.exception(
                    "Unexpected error publishing story",
                    extra={
                        "story_id": story_id,
                        "iteration": cls._iteration_count,
                    },
        ...
        try:
            ...
            if "access_token" in data:
                ...
                if new_token != access_token:
                    ...
                    logger.info(
                        "Instagram token refreshed and saved",
                        extra={
    ...
    async def _collect_pending_metrics(cls, admin: Client, config: dict) -> None:
        ...
        for post in posts:
            ...
            try:
                ...
                logger.warning(
                    "Failed to collect metrics for post",
                    extra={
                        "post_id": post["id"],
                        "iteration": cls._iteration_count,
                    },
