...
class CleanupService:
    ...
    async def execute(
        ...
        try:
            await (
                admin_supabase.table("audit_log")
                .insert(
                    {
                        "entity_type": "cleanup",
                        "entity_id": None,
    ...
    async def _get_matching_epochs(
        ...
        query = admin_supabase.table("game_epochs").select("id, name, updated_at").eq("status", status)
        if epoch_ids:
    ...
    async def _count_cascade_targets(
        ...
        for table in _EPOCH_CASCADE_TABLES:
            resp = await (
                admin_supabase.table(table).select("id", count="exact").in_("epoch_id", epoch_ids).limit(0).execute()
            )
    ...
    async def _get_matching_archived(
        ...
        query = admin_supabase.table("simulations").select("id, name, updated_at").eq("simulation_type", "archived")
        if instance_ids:
