...
Covers:
  ...
  dungeon_encounters:
    ...
    - _shadow_encounters(): 11 encounters, bilingual, choices
    - _tower_encounters(): 11 encounters, bilingual, choices
  dungeon_loot:
...
# Content-service accessors (pack-backed cache, A1.4+). Tests now read via
# these getters instead of the deleted Python dict registries; data-integrity
# invariants beyond what we assert here are enforced by the pack validator
# (`scripts/validate_content_packs.py`).


def _shadow_enemies():
    return get_enemy_registry()["The Shadow"]


def _tower_enemies():
    return get_enemy_registry()["The Tower"]


def _shadow_spawns():
    return get_spawn_registry()["The Shadow"]


def _tower_spawns():
    return get_spawn_registry()["The Tower"]


def _shadow_encounters():
    return get_encounter_registry()["The Shadow"]


def _tower_encounters():
    return get_encounter_registry()["The Tower"]


def _shadow_banter():
    return get_banter_registry()["The Shadow"]


def _tower_banter():
    return get_banter_registry()["The Tower"]


def _shadow_loot_tier(tier):
    return get_loot_registry()["The Shadow"][tier]
...
class TestShadowEnemies:
    ...
    def test_expected_ids(self):
        assert set(_shadow_enemies().keys()) == self.EXPECTED_IDS

    def test_all_have_bilingual_names(self):
        for eid, tmpl in _shadow_enemies().items():
            assert tmpl.name_en, f"{eid} missing name_en"
            assert tmpl.name_de, f"{eid} missing name_de"

    def test_threat_levels(self):
        assert _shadow_enemies()["shadow_wisp"].threat_level == "minion"
        assert _shadow_enemies()["shadow_tendril"].threat_level == "minion"
        assert _shadow_enemies()["shadow_echo_violence"].threat_level == "standard"
        assert _shadow_enemies()["shadow_paranoia_shade"].threat_level == "standard"
        assert _shadow_enemies()["shadow_remnant"].threat_level == "elite"

    def test_paranoia_shade_lies(self):
        """Paranoia Shade has telegraphed_intent=False — it lies about intents."""
        assert _shadow_enemies()["shadow_paranoia_shade"].telegraphed_intent is False
...
class TestSpawnEnemies:
    ...
    def test_all_spawn_configs_produce_instances(self):
        for config_id in _shadow_spawns():
...
class TestEncounterData:
    def test_encounter_count(self):
        assert len(_shadow_encounters()) == 13

    def test_encounter_type_distribution(self):
        types = {}
        for e in _shadow_encounters():
    ...
    def test_all_ids_unique(self):
        ids = [e.id for e in _shadow_encounters()]
        assert len(ids) == len(set(ids))
...
class TestGetEncounterById:
    ...
    @pytest.mark.parametrize(
        "encounter_id",
        [e.id for e in _shadow_encounters()],
    )
...
class TestSelectBanter:
    ...
    def test_excludes_used_ids(self):
        ...
        all_ids = [b["id"] for b in _shadow_banter() if b["trigger"] == "room_entered"]
...
class TestLootTables:
    def test_tier_1_has_4_items(self):
        assert len(_shadow_loot_tier(1)) == 8

    def test_tier_2_has_5_items(self):
        assert len(_shadow_loot_tier(2)) == 5

    def test_tier_3_has_3_items(self):
        assert len(_shadow_loot_tier(3)) == 3

    def test_all_items_have_bilingual_text(self):
        for tier, items in get_loot_registry()["The Shadow"].items():
            for item in items:
    ...
    def test_all_ids_unique(self):
        all_ids = []
        for items in get_loot_registry()["The Shadow"].values():
    ...
    def test_shadow_attunement_cap_review_20(self):
        ...
        attunement = next(i for i in _shadow_loot_tier(3) if i.id == "shadow_attunement")
...
class TestTowerEnemies:
    ...
    def test_expected_ids(self):
        ...
        assert set(_tower_enemies().keys()) == self.EXPECTED_IDS

    def test_all_have_bilingual_names(self):
        """Every Tower enemy has both name_en and name_de."""
        for eid, tmpl in _tower_enemies().items():
            assert tmpl.name_en, f"{eid} missing name_en"
            assert tmpl.name_de, f"{eid} missing name_de"
    ...
    def test_threat_levels(self):
        ...
        assert _tower_enemies()["tower_tremor_broker"].threat_level == "minion"
        assert _tower_enemies()["tower_foundation_worm"].threat_level == "minion"
        assert _tower_enemies()["tower_crown_keeper"].threat_level == "standard"
        assert _tower_enemies()["tower_debt_shade"].threat_level == "standard"
        assert _tower_enemies()["tower_remnant_commerce"].threat_level == "elite"

    def test_threat_level_counts(self):
        """Verify exact count: 2 minion, 2 standard, 1 elite."""
        levels = [t.threat_level for t in _tower_enemies().values()]
    ...
    def test_debt_shade_lies(self):
        ...
        assert _tower_enemies()["tower_debt_shade"].telegraphed_intent is False
    ...
    def test_registry_contains_tower(self):
        """Tower enemies are registered in the get_enemy_registry()."""
        assert "The Tower" in get_enemy_registry()
        assert get_enemy_registry()["The Tower"] is _tower_enemies()
...
class TestTowerSpawnConfigs:
    ...
    def test_expected_config_ids(self):
        ...
        assert set(_tower_spawns().keys()) == self.EXPECTED_CONFIG_IDS
    ...
    def test_all_template_ids_reference_valid_tower_enemies(self):
        ...
        for config_id, entries in _tower_spawns().items():
            for entry in entries:
                assert entry["template_id"] in _tower_enemies(), (
                    f"{config_id} references unknown template {entry['template_id']}"
                )

    def test_all_spawn_configs_produce_instances(self):
        """Every spawn config produces at least one enemy instance."""
        for config_id in _tower_spawns():
    ...
    def test_registry_contains_tower(self):
        """Tower spawn configs are registered in the get_spawn_registry()."""
        assert "The Tower" in get_spawn_registry()
        assert get_spawn_registry()["The Tower"] is _tower_spawns()
...
class TestGetEnemyTemplatesDictTower:
    ...
    def test_keys_match_tower_enemies(self):
        """Dict keys match _tower_enemies() keys."""
        result = get_enemy_templates_dict(archetype="The Tower")
        assert set(result.keys()) == set(_tower_enemies().keys())
...
class TestTowerEncounters:
    ...
    def test_all_ids_unique(self):
        ...
        ids = [e.id for e in _tower_encounters()]
        assert len(ids) == len(set(ids))
    ...
    def test_boss_encounter_has_combat_encounter_id(self):
        ...
        assert boss.combat_encounter_id in _tower_spawns()
    ...
    def test_elite_encounter_has_combat_encounter_id(self):
        ...
        assert elite.combat_encounter_id in _tower_spawns()
    ...
    def test_combat_encounters_have_combat_encounter_ids(self):
        ...
        for e in _tower_encounters():
            if e.room_type == "combat":
                ...
                assert e.combat_encounter_id in _tower_spawns(), (
                    f"{e.id} references unknown spawn config {e.combat_encounter_id}"
                )

    def test_registry_contains_tower(self):
        """Tower encounters are registered in the get_encounter_registry()."""
        assert "The Tower" in get_encounter_registry()
        assert get_encounter_registry()["The Tower"] is _tower_encounters()

    @pytest.mark.parametrize(
        "encounter_id",
        [e.id for e in _tower_encounters()],
    )
...
class TestSelectBanterTower:
    ...
    def test_excludes_used_ids(self):
        ...
        all_ids = [b["id"] for b in _tower_banter() if b["trigger"] == "room_entered"]
...
class TestTowerBanterData:
    ...
    def test_registry_contains_tower(self):
        """Tower banter is registered in the get_banter_registry()."""
        assert "The Tower" in get_banter_registry()
        assert get_banter_registry()["The Tower"] is _tower_banter()

    def test_elite_spotted_trigger_exists(self):
        """Tower has elite_spotted banter."""
        matches = [b for b in _tower_banter() if b["trigger"] == "elite_spotted"]
        assert len(matches) >= 1

    def test_agent_afflicted_trigger_exists(self):
        """Tower has agent_afflicted banter."""
        matches = [b for b in _tower_banter() if b["trigger"] == "agent_afflicted"]
        assert len(matches) >= 1

    def test_agent_virtue_trigger_exists(self):
        """Tower has agent_virtue banter."""
        matches = [b for b in _tower_banter() if b["trigger"] == "agent_virtue"]
        assert len(matches) >= 1
...
class TestTowerLootTables:
    ...
    def test_tier_1_has_4_items(self):
        assert len(_tower_loot_tier(1)) == 8

    def test_tier_2_has_5_items(self):
        assert len(_tower_loot_tier(2)) == 5

    def test_tier_3_has_3_items(self):
        assert len(_tower_loot_tier(3)) == 3

    def test_all_items_have_bilingual_text(self):
        """Every Tower loot item has bilingual name and description."""
        for tier, items in get_loot_registry()["The Tower"].items():
            for item in items:
                assert item.name_en, f"Tier {tier} item {item.id} missing name_en"
                assert item.name_de, f"Tier {tier} item {item.id} missing name_de"
    ...
    def test_all_ids_unique(self):
        ...
        all_ids: list[str] = []
        for items in get_loot_registry()["The Tower"].values():
            all_ids.extend(i.id for i in items)
        assert len(all_ids) == len(set(all_ids))
    ...
    def test_foundation_attunement_cap_review_20(self):
        ...
        attunement = next(i for i in _tower_loot_tier(3) if i.id == "foundation_attunement")
        assert attunement.effect_params["max_total_bonus"] == 2
    ...
    def test_tier_3_all_guaranteed(self):
        ...
        guaranteed = [i for i in _tower_loot_tier(3) if i.drop_weight == 100]
    ...
    def test_stability_catalyst_exists(self):
        ...
        catalyst = next((i for i in _tower_loot_tier(3) if i.id == "stability_catalyst"), None)
        assert catalyst is not None
        assert catalyst.effect_type == "simulation_modifier"

    def test_registry_contains_tower(self):
        """Tower loot tables are registered in the get_loot_registry()."""
        assert "The Tower" in get_loot_registry()
        assert get_loot_registry()["The Tower"] is get_loot_registry()["The Tower"]
