-- Add image_generated_at to simulation_lore for tracking lore image generation progress
ALTER TABLE public.simulation_lore
  ADD COLUMN IF NOT EXISTS image_generated_at timestamptz DEFAULT NULL;

-- Extend get_forge_progress() to include lore image progress and banner_url
...
BEGIN
  -- Resolve simulation
  SELECT id, banner_url INTO v_sim_id, v_banner
  FROM simulations
  WHERE slug = p_slug
  LIMIT 1;
