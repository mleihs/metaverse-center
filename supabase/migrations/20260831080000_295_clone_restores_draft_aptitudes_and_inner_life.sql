-- Migration 295: Der Epochen-Klon verwarf die Vorbereitung der Spielenden.
--
-- Befund I der Systempruefung, plus zwei Funde beim Lesen des Prod-Koerpers.
--
-- WAS DER KLON VERLOR
--
-- 1. DEN DRAFT. Die Agentenauswahl lautete "ORDER BY created_at LIMIT 6" --
--    die sechs aeltesten Agenten der Welt. `epoch_participants.drafted_agent_ids`
--    steht in derselben Zeile wie die `simulation_id`, die die Schleife liest,
--    und wurde nicht ausgewaehlt. Der Draft ist die einzige Vorbereitungs-
--    entscheidung vor einer Epoche; sie wurde beim Start still durch eine
--    Sortierung ersetzt. Nichts schlug fehl: die Epoche startete mit sechs
--    Agenten, nur eben nicht mit den gewaehlten.
--
-- 2. DIE EIGNUNGEN. `agent_aptitudes` wurde gar nicht geklont. Jeder Agent in
--    einer Epoche startete daher als ebener Generalist -- die Herleitung liest
--    eine leere Tabelle und liefert fuer alle denselben Wert.
--
-- 3. DAS INNENLEBEN. Weder `agent_mood` noch `agent_needs` noch eine Startzone.
--    Genau der Zustand, den Migration 286 (A3) beim Schmieden beseitigt hat --
--    hier eine Ebene weiter hinten unangetastet geblieben.
--
-- 4. DEN DEUTSCHEN WELTTITEL. Die Spaltenliste des Simulations-INSERT lautete
--    `name, slug, description, description_de, ...` -- `name_de` fehlte,
--    waehrend `description_de` direkt daneben mitkam. (Gefunden beim Lesen,
--    nicht in den Daten: alle 20 Klone auf Prod stammen aus Vorlagen, die
--    selbst keinen deutschen Titel haben.)
--
-- 5. DAS WESEN. `personality_profile` wurde nicht mitgeklont. Seit Migration
--    296 der Parallelsitzung ist das Feld nicht mehr durchgehend leer, also
--    haette der Klon ab jetzt aktiv etwas weggeworfen.
--
-- WIE DAS INNENLEBEN ANGELEGT WIRD, UND WARUM DAS DER HEIKLE TEIL IST
--
-- `fn_initialize_agent_autonomy` traegt zweimal `ON CONFLICT (agent_id) DO
-- NOTHING`. Wer die Zeile mit den Signaturvorgaben anlegt, entscheidet damit
-- zugleich, dass NIE ein anderer Wert hineinkommt -- ein spaeterer Aufruf mit
-- echten Werten tut nichts. Das ist der Befund, den die Parallelsitzung beim
-- Bau von 296 gemacht hat.
--
-- Beim Klonen ist die richtige Antwort aber schon bekannt: sie steht beim
-- Ursprungsagenten. Diese Migration uebergibt sie deshalb BEIM ANLEGEN, mit
-- COALESCE auf die Signaturvorgaben fuer Ursprungsagenten ohne Zeile. Die
-- Fuelleragenten (wenn eine Welt weniger als sechs Agenten hat) bekommen die
-- Vorgaben -- bei ihnen sind sie die richtige Antwort, nicht der Rueckfall.
--
-- DROP STATT CREATE OR REPLACE
--
-- Der Koerper stammt aus `pg_get_functiondef` auf Prod (nicht aus einer alten
-- Migration -- der Ledger taugt nicht als Beleg fuer den Bestand). Die
-- Signatur bleibt unveraendert, aber DROP + CREATE ist hier die sichere Form:
-- `CREATE OR REPLACE` wuerde bei einer abweichenden Argumentliste stillschweigend
-- eine UEBERLADUNG anlegen, und dann entschiede der Aufrufer per Typ, welche
-- der beiden Fassungen laeuft. Gemessen: genau eine Ueberladung auf Prod
-- (`p_epoch_id uuid, p_created_by_id uuid, p_epoch_number integer`).
--
-- Rechte danach wiederhergestellt. Gemessener Ist-Zustand auf Prod:
--   {postgres=X/postgres, service_role=X/postgres}
-- Also KEIN Grant an anon oder authenticated -- die Funktion ist SECURITY
-- DEFINER und darf nur ueber den service_role-Client hinter der
-- Rollenpruefung erreichbar sein (CLAUDE.md, ADR-006).

BEGIN;

DROP FUNCTION IF EXISTS public.clone_simulations_for_epoch(uuid, uuid, integer);

CREATE FUNCTION public.clone_simulations_for_epoch(p_epoch_id uuid, p_created_by_id uuid, p_epoch_number integer DEFAULT 1)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO simulations (
      name, name_de, slug, description, description_de, theme, status, content_locale,
      additional_locales, owner_id, icon_url, banner_url,
      simulation_type, source_template_id, epoch_id
    ) VALUES (
      sim.name || ' (Epoch ' || p_epoch_number || ')',
      -- name_de fehlte in dieser Liste. Der Klon verlor den deutschen Titel
      -- der Welt, waehrend description_de direkt daneben mitkam.
      CASE
        WHEN sim.name_de IS NULL OR sim.name_de = '' THEN NULL
        ELSE sim.name_de || ' (Epoche ' || p_epoch_number || ')'
      END,
      new_slug,
      sim.description,
      sim.description_de,
    ...
    --
    -- Die Auswahl folgt dem DRAFT, nicht dem Alter. Vorher stand hier
    -- "ORDER BY created_at LIMIT 6" -- die sechs aeltesten Agenten der Welt,
    -- unabhaengig davon, wen die Spielerin fuer diese Epoche gezogen hatte.
    -- Der Draft ist die einzige Vorbereitungsentscheidung vor einer Epoche;
    -- sie wurde beim Start still verworfen und durch eine Sortierung ersetzt.
    -- Nichts schlug fehl, und niemand konnte es sehen: die Epoche startete
    -- mit sechs Agenten, nur eben nicht mit den gewaehlten.
    --
    -- array_position erhaelt dabei die REIHENFOLGE des Drafts: der erste Zug
    -- bleibt der erste Agent. Ein reines "= ANY(...)" haette die richtige
    -- Menge in fremder Reihenfolge geklont.
    --
    -- Rueckfall auf created_at, wenn kein Draft vorliegt (Epochen ohne
    -- Draftphase, Altbestand): dort ist das bisherige Verhalten das richtige.
    agent_clone_count := 0;
    FOR old_id IN
      SELECT a.id FROM agents a
      WHERE a.simulation_id = sim.id
        AND a.deleted_at IS NULL
        AND (
          participant.drafted_agent_ids IS NULL
          OR array_length(participant.drafted_agent_ids, 1) IS NULL
          OR a.id = ANY(participant.drafted_agent_ids)
        )
      ORDER BY
        CASE
          WHEN participant.drafted_agent_ids IS NULL THEN NULL
          ELSE array_position(participant.drafted_agent_ids, a.id)
        END NULLS LAST,
        a.created_at
      LIMIT 6
    LOOP
      ...
      -- A8b: Eignungen mitklonen.
      -- Ohne diese Zeilen startet jeder geklonte Agent als ebener
      -- Generalist: die Eignungsherleitung liest eine leere Tabelle und
      -- liefert fuer alle denselben Wert. Die Aufstellung, die jemand ueber
      -- Wochen aufgebaut hat, endet in der Epoche als Einheitsbrei -- und
      -- zwar unsichtbar, weil nichts fehlschlaegt.
      INSERT INTO agent_aptitudes (agent_id, simulation_id, operative_type, aptitude_level)
      SELECT new_id, new_sim_id, operative_type, aptitude_level
      FROM agent_aptitudes WHERE agent_id = old_id;

      -- A8c: Innenleben mitklonen.
      --
      -- Wichtig ist, WIE: nicht mit den Signaturvorgaben anlegen und danach
      -- korrigieren. fn_initialize_agent_autonomy traegt zweimal
      -- "ON CONFLICT (agent_id) DO NOTHING" -- ein zweiter Aufruf taete also
      -- nichts. Wer die Zeile mit Vorgaben anlegt, entscheidet damit, dass
      -- nie ein anderer Wert hineinkommt (Befund der Parallelsitzung zu N1).
      --
      -- Beim Klonen sind die richtigen Werte aber SCHON BEKANNT: sie stehen
      -- beim Ursprungsagenten. Also werden sie beim Anlegen uebergeben.
      -- COALESCE faellt auf die Signaturvorgaben zurueck, falls der Ursprung
      -- selbst keine Zeile hat (Welten von vor der Rueckfuellung).
      PERFORM fn_initialize_agent_autonomy(
        new_id,
        new_sim_id,
        COALESCE((SELECT m.resilience  FROM agent_mood  m WHERE m.agent_id = old_id), 0.5::real),
        COALESCE((SELECT m.volatility  FROM agent_mood  m WHERE m.agent_id = old_id), 0.5::real),
        COALESCE((SELECT m.sociability FROM agent_mood  m WHERE m.agent_id = old_id), 0.5::real),
        COALESCE((SELECT n.social_decay      FROM agent_needs n WHERE n.agent_id = old_id), 5.0::real),
        COALESCE((SELECT n.purpose_decay     FROM agent_needs n WHERE n.agent_id = old_id), 3.0::real),
        COALESCE((SELECT n.safety_decay      FROM agent_needs n WHERE n.agent_id = old_id), 2.0::real),
        COALESCE((SELECT n.comfort_decay     FROM agent_needs n WHERE n.agent_id = old_id), 2.0::real),
        COALESCE((SELECT n.stimulation_decay FROM agent_needs n WHERE n.agent_id = old_id), 4.0::real)
      );
    ...
    WHILE agent_clone_count < 6 LOOP
      ...
      -- Auch die Fuelleragenten bekommen ein Innenleben. Sie sind erfunden,
      -- es gibt also keine Ursprungswerte -- die Signaturvorgaben sind hier
      -- die richtige Antwort und nicht bloss der Rueckfall. Ohne diesen
      -- Aufruf haette eine Epoche Agenten ohne Stimmung, Beduerfnisse und
      -- Startzone, also genau den Zustand, den A3 beim Schmieden beseitigt
      -- hat -- nur eine Ebene weiter hinten wieder eingefuehrt.
      PERFORM fn_initialize_agent_autonomy(new_id, new_sim_id);
    END LOOP;
...
-- Rechte exakt wie vor dem DROP. DROP FUNCTION nimmt die Grants mit; ohne
-- diese Zeilen liefe der Epochenstart danach in ein "permission denied", und
-- zwar erst beim naechsten echten Start, nicht bei der Migration.
REVOKE ALL ON FUNCTION public.clone_simulations_for_epoch(uuid, uuid, integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.clone_simulations_for_epoch(uuid, uuid, integer) FROM anon;
REVOKE ALL ON FUNCTION public.clone_simulations_for_epoch(uuid, uuid, integer) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.clone_simulations_for_epoch(uuid, uuid, integer) TO service_role;
