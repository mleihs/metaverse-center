-- Migration 127: Scoring engine RPC — push 12 queries per simulation into 1 SQL call.
--
-- fn_compute_cycle_scores(p_epoch_id, p_cycle_number, p_score_weights)
-- Computes raw scores across 5 dimensions for all participants,
-- normalises them (max-per-dimension), applies weighted composite,
-- and upserts into epoch_scores. Returns the final rows.
...
BEGIN
    ...
    -- Zone stability (avg per sim from mv_zone_stability)
    zone_stab AS (
    ...
    -- Outbound missions (attacks from each sim)
    outbound_missions AS (
        SELECT p.simulation_id,
               ...
               -- Military: mission values - detection penalties
               COALESCE(SUM(CASE
    ...
    raw_scores AS (
        SELECT p.simulation_id,
               ...
               * (1.0 - COALESCE(td.betrayal_penalty, 0))
               AS diplomatic,
               -- Military: sum(mission_values) - detection_penalties, floor 0
...
-- Grant execute to authenticated users (RLS on epoch_scores still enforced)
