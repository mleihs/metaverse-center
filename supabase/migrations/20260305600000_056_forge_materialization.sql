-- 056: Simulation Forge Materialization Logic
-- Implements the atomic SQL function to move drafts into production.
...
BEGIN
    ...
    IF v_draft.status != 'draft' THEN
        RAISE EXCEPTION 'Draft is already processed or in progress.';
    END IF;
    ...
    LOOP
        ...
        SELECT v_sim_id, rtrim(v_setting_key, 's'), val, jsonb_build_object('en', val)
    ...
    -- 12. Insert Buildings (round-robin distribution across zones)
    FOR v_building IN SELECT * FROM jsonb_array_elements(v_draft.buildings)
    LOOP
        INSERT INTO public.buildings (simulation_id, city_id, zone_id, name, building_type, description)
        VALUES (
            v_sim_id,
            v_city_id,
