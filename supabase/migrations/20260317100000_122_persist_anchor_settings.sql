-- Migration 122: Persist philosophical anchor data to simulation_settings.
--
-- Previously only title → simulations.name and description → simulations.description
-- were kept from the anchor. All other fields (core_question, literary_influence,
-- bleed_signature_suggestion, all _de translations, seed_prompt) were lost once the
-- draft was deleted. Now fn_materialize_shard writes them to simulation_settings
-- with category='anchor' in the same transaction.
--
-- Also widens the anon RLS policy to include category='anchor' so the public
-- endpoint can read anchor data without auth.

-- ── 1. Extend fn_materialize_shard ──────────────────────────────────────
...
-- ── 2. Widen anon RLS policy to include 'anchor' category ───────────────
