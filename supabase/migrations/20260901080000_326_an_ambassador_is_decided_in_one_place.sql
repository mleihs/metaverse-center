-- ============================================================================
-- Migration 326 — Wer ein Botschafter ist, wird an EINER Stelle entschieden
--
-- ⚠ UMNUMMERIERT VON 322 AM 31.08.2026 (von `velgarien-rebuild-88`, ungefragt,
-- weil es eilte). Diese Datei trug den Zeitstempel `20260901040000` — denselben
-- wie `322_one_function_answered_two_questions`. `version` ist der
-- Primärschlüssel von `supabase_migrations.schema_migrations`: beide Dateien
-- konkurrierten um EINE Zeile, und die andere hatte sie bereits.
--
-- Die Wirkung dieser Migration war zu dem Zeitpunkt auf Prod (`active_ambassadors`
-- und `fn_compute_agent_influence` nachgemessen), aber OHNE Ledger-Zeile: beim
-- nächsten Migrationslauf hätte das Werkzeug den Zeitstempel als erledigt
-- gesehen und diese Datei stillschweigend übersprungen. Der Ledger-Eintrag ist
-- unter `20260901080000` nachgetragen.
--
-- Inhaltlich ist NICHTS geändert. Die beiden Migrationen sind voneinander
-- unabhängig (Botschafter-Identität gegen Bauzustandsleiter), die Reihenfolge
-- spielt keine Rolle.
-- ============================================================================
--
-- BEFUND
