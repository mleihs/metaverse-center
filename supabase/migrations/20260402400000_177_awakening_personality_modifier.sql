-- ============================================================================
-- Migration 177: personality_modifier support for Awakening Insight loot
--
-- Adds personality_modifier branch to fn_apply_dungeon_loot.
-- Effect: modify one Big Five dimension by ±delta (stored in personality_profile JSONB).
-- Two-phase: immediate apply (dimension chosen by caller) + audit trail in loot_effects.
-- ============================================================================
...
BEGIN
  ...
  -- Params: {"dimension": "openness", "delta": 0.1}
  -- Dimension must be one of: openness, conscientiousness, extraversion, agreeableness, neuroticism
  ...
  ELSIF v_effect_type = 'personality_modifier' THEN
    ...
    IF v_dimension IS NULL OR v_dimension NOT IN (
      ...
      RETURN;  -- invalid dimension, skip silently
    END IF;

    -- Read current value from personality_profile JSONB
    ...
    -- Atomic update of the single dimension within the JSONB
    UPDATE agents
    ...
    -- Audit trail: record the modification
