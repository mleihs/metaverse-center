...
export class VelgHowToPlay extends LitElement {
    ...
    return html`
      ...
      <div class="layout">
        ...
        <main class="content">
          ...
          ${this._renderBureauTerminalGuide()}
          <!-- Reference -->
          ${this._renderTactics()}
          ${this._renderDemoRun()}
          ${this._renderMatches()}
  ...
  /* ── Section 15: The Simulation Forge ────────────── */

  private _renderForgeGuide() {
    const steps = getForgeGuideSteps();
    const phases: ForgeStep['phase'][] = ['astrolabe', 'table', 'darkroom', 'ignition'];
    const phaseLabels: Record<ForgeStep['phase'], string> = {
      astrolabe: msg('Astrolabe'),
      table: msg('Table'),
      darkroom: msg('Darkroom'),
    ...
    return html`
      <section class="section" id="forge-guide">
        ...
        <div class="demo-steps">
          ${steps.map((step, i) => this._renderDemoStep(step as unknown as DemoStep, i))}
        </div>
      </section>
    `;
  }

  /* ── Section 16: The Chronicle ──────────────────── */

  private _renderChronicleGuide() {
  ...
  /* ── Section 16b: Bureau Terminal ────────────────── */

  private _renderBureauTerminalGuide() {
    const steps = getBureauTerminalGuideSteps();
    return html`
      <section class="section" id="bureau-terminal">
        ${this._renderSectionHeader('25', msg('Bureau Terminal'))}
        <p class="section__text">
          ${msg('A text-based command interface that places you inside the simulation. Type commands, interrogate agents, gather intelligence, and manage zone operations through narrative prose. Three clearance tiers unlock progressively as you gain experience: Observation, Field Operations, and Intelligence Network.')}
        </p>
        <div class="demo-steps">
          ${steps.map((step, i) => this._renderDemoStep(step, i))}
        </div>
  ...
  /* ── Section 17: Substrate Resonances ───────────── */

  private _renderResonanceGuide() {
  ...
  /* ── Section 18: Event Pressure & Zones ─────────── */

  private _renderZoneDynamicsGuide() {
  ...
  /* ── Section 19: Agent Memory ───────────────────── */

  private _renderAgentMemoryGuide() {
