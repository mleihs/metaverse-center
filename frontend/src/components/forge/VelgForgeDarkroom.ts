...
export class VelgForgeDarkroom extends LitElement {
  ...
  connectedCallback() {
    ...
    // Auto-generate theme on entry if stale (entities changed or no theme yet).
    // Change detection lives in ForgeStateManager.themeStale (computed signal).
    if (forgeStateManager.themeStale.value && !forgeStateManager.isGeneratingTheme.value) {
      this._generateTheme();
    }
  }
